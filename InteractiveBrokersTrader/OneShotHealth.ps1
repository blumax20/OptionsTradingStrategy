<# =======================================================================
 ...
 - Summarizes IBKR P/L (day, YTD baseline) using ImprovedHealthQueries.py
 - Tails recent logs
 ======================================================================= #>
  ...
  if ($rows) {
    ...
    foreach ($r in $rows) {
      ...
      if ($r.NeedsDaily -and -not ($r.CallsDaily -or $indirect)) {
        "    WARNING: Expected DailyCycleManagement.py (task is not calling it)" |
          Tee-Object -FilePath $Report -Append | Out-Null
      }
    }
  } else {
...
# ---------- Listening Ports ----------
"--- Listening Ports (5001, 7497, 7496) ---" | Tee-Object -FilePath $Report -Append
try {
...
def main():
    ib = IB()
    try:
        ib.connect('127.0.0.1', 7497, clientId=900, timeout=6)
    except Exception as e:
...
def main():
    ib=IB(); ib.connect('127.0.0.1',7497,clientId=897,timeout=6)
    poss = ib.positions()
...
def main():
    ib=IB()
    try:
        ib.connect('127.0.0.1',7497,clientId=901,timeout=6)
    except Exception as e:
