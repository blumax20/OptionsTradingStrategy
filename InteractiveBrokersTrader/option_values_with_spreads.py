...
option prices for strike prices spaced at a configurable increment (default $1, e.g. $0.5 or $1)
around the last price (±10 strikes) for three time‑to‑maturity horizons: 15, 30 and 60 days. In addition, it calculates
the values of **multiple debit spread widths** (e.g., $0.5, $1, $2.5, $5) using the strike closest
to at‑the‑money (ATM) and the appropriate offset legs up for calls and down for puts. All results
are written to an Excel file with separate sheets for the option prices and the debit spreads.
The risk‑free rate and dividend yield are assumed constant (4.5 % and 0 %, respectively), but these
can be adjusted in the `main` function, or you can pass CLI flags to choose spread widths and strike step.
...
Example input prompt and output:
    ...
    Debit spread values (by width):
     Maturity Call_Spread_0.5 Put_Spread_0.5 Call_Spread_1.0 Put_Spread_1.0 Call_Spread_2.5 Put_Spread_2.5 Call_Spread_5.0 Put_Spread_5.0
         15d           0.27          0.24           0.54          0.47           1.32          1.14           2.52          2.18
         30d           0.34          0.29           0.69          0.58           1.68          1.45           3.17          2.74
         60d           0.45          0.38           0.91          0.76           2.20          1.89           4.13          3.54
...
def parse_market_data(csv_path: str) -> Dict[str, float]:
    ...
    rets = df.loc[df['date'] <= last_date, 'log_ret'].dropna()
    if len(rets) >= 2:
        sigma = float(rets.tail(20).std(ddof=1) * math.sqrt(252))
    else:
        sigma = float('nan')  # insufficient data; user may override via CLI input later
...
def build_option_table(
    ...
    # Anchor to nearest strike_step (e.g., $0.50 if strike_step=0.5)
    base_strike = round(S / strike_step) * strike_step
    strike_start = max(strike_step, base_strike - strike_span)
...
def compute_debit_spreads(
    ...
    """
    # Anchor to the nearest $0.50 to better align with common strike ladders
    base_strike = round(S * 2) / 2.0
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Option values and debit spreads tool")
    parser.add_argument("--increments", default="0.5,1,2.5,5",
                        help="Comma-separated spread widths (e.g., '0.5,1,2.5,5').")
    parser.add_argument("--strike-step", type=float, default=1.0,
                        help="Strike grid step for the pricing table (e.g., 0.5 for $0.50).")
    ...
    option_table = build_option_table(S, sigma, r, strike_step=args.strike_step)
    ...
    try:
        increments = [float(x) for x in args.increments.split(",") if x.strip() != ""]
    except Exception:
