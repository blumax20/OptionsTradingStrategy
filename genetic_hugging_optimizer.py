...
def bollinger_bands(series: pd.Series, length: int, std_dev: float) -> Tuple[pd.Series, pd.Series, pd.Series]:
    ...
    basis = series.rolling(window=length, min_periods=length).mean()
...
def stochastic_rsi(rsi: pd.Series, length: int, smooth: int) -> pd.Series:
    ...
    rsi_min = rsi.rolling(window=length, min_periods=length).min()
    rsi_max = rsi.rolling(window=length, min_periods=length).max()
    stoch = 100 * (rsi - rsi_min) / (rsi_max - rsi_min + 1e-9)
    return stoch.rolling(window=smooth, min_periods=smooth).mean()
...
def money_flow_index(high: pd.Series, low: pd.Series, close: pd.Series, volume: pd.Series, length: int) -> pd.Series:
    ...
    pos_sum = pd.Series(positive_flow).rolling(window=length, min_periods=length).sum()
    neg_sum = pd.Series(negative_flow).rolling(window=length, min_periods=length).sum()
...
def average_directional_index(high: pd.Series, low: pd.Series, close: pd.Series, length: int) -> pd.Series:
    ...
    plus_di = 100 * pd.Series(plus_dm).rolling(window=length, min_periods=length).sum() / (atr + 1e-9)
    minus_di = 100 * pd.Series(minus_dm).rolling(window=length, min_periods=length).sum() / (atr + 1e-9)
...
# ---------------------------------------------------------------------------
def compute_fitness(df: pd.DataFrame, params: Dict[str, Union[int, float]]) -> Tuple[float, float, int]:
    ...
    # Bollinger Bands
    basis, upper, lower = bollinger_bands(close, int(params['bbLength']), float(params['bbStdDev']))
    width = upper - lower
    bb_width = width / basis
    bb_width_ma = bb_width.rolling(window=20, min_periods=20).mean()

    # RSI and Stochastic RSI
    rsi = wilder_rsi(close, int(params['stochLength']))
    k = stochastic_rsi(rsi, int(params['stochLength']), int(params['stochSmooth']))

    # Money Flow Index
    mfi = money_flow_index(high, low, close, volume, int(params['mfiLength']))

    # MACD components
    ema_fast = exponential_moving_average(close, int(params['macdFast']))
    ema_slow = exponential_moving_average(close, int(params['macdSlow']))
    macd_line = ema_fast - ema_slow
    signal_line = exponential_moving_average(macd_line, int(params['macdSignal']))

    # ATR and ADX
    atr = average_true_range(high, low, close, int(params['atrLength']))
    adx = average_directional_index(high, low, close, int(params['adxLength']))

    # Warmup: find first index where all indicators are valid
    valid = (~basis.isna() & ~upper.isna() & ~lower.isna() &
             ~k.isna() & ~mfi.isna() & ~ema_fast.isna() & ~ema_slow.isna() &
             ~signal_line.isna() & ~atr.isna() & ~adx.isna())
    if not valid.any():
        return (0.0, 0.0)
    start_idx = int(np.argmax(valid.values))
    ...
    # Iterate over bars (after warmup)
    for idx in range(start_idx, len(df)):
        ...
        MFI_OS = int(params['MFI_OVERSOLD'])
        MFI_OB = int(params['MFI_OVERBOUGHT'])
        ...
        hug_lower = lower_band + width_i * float(params['hugPct'])
        hug_upper = upper_band - width_i * float(params['hugPct'])
        ...
        # Trend filters
        trend_bull = (ema_fast.iloc[idx] >= ema_slow.iloc[idx]) or (adx.iloc[idx] < float(params['adxThreshold']))
        trend_bear = (ema_fast.iloc[idx] <= ema_slow.iloc[idx]) or (adx.iloc[idx] < float(params['adxThreshold']))
        bull_condition = (bull_score >= int(params['MIN_SCORE'])) and trend_bull
        bear_condition = (bear_score >= int(params['MIN_SCORE'])) and trend_bear
        current_bar = idx
        ...
        if position > 0:
            ...
            if (hug_lower_count >= params['hugBars']) and (ema_fast.iloc[idx] < ema_slow.iloc[idx]):
                ...
                put_take_profit = price - float(params['profitFactor']) * atr.iloc[idx]
                put_stop_loss = price + float(params['stopFactor']) * atr.iloc[idx]
                # Reset long state
                call_entry_bar = None
                call_entry_price = None
                call_take_profit = None
            ...
            time_exceeded = (call_entry_bar is not None and current_bar >= call_entry_bar + int(params['EXIT_BARS']))
            extended_limit = (call_entry_bar is not None and current_bar >= call_entry_bar + int(params['EXIT_BARS']) * 2)
            should_exit_due_time = time_exceeded and (
                (ema_fast.iloc[idx] < ema_slow.iloc[idx] and adx.iloc[idx] >= float(params['adxThreshold'])) or extended_limit
            )
        ...
        # Manage open short position
            ...
            if (hug_upper_count >= params['hugBars']) and (ema_fast.iloc[idx] > ema_slow.iloc[idx]):
                ...
                call_take_profit = price + float(params['profitFactor']) * atr.iloc[idx]
                call_stop_loss = price - float(params['stopFactor']) * atr.iloc[idx]
                # Reset short state
                put_entry_bar = None
                put_entry_price = None
                put_take_profit = None
            ...
            time_exceeded = (put_entry_bar is not None and current_bar >= put_entry_bar + int(params['EXIT_BARS']))
            extended_limit = (put_entry_bar is not None and current_bar >= put_entry_bar + int(params['EXIT_BARS']) * 2)
            should_exit_due_time = time_exceeded and (
                (ema_fast.iloc[idx] > ema_slow.iloc[idx] and adx.iloc[idx] >= float(params['adxThreshold'])) or extended_limit
            )
    ...
    if total_trades < 1:
        ...
        pop = profitable_trades / total_trades if total_trades >= 12 else 0.0
    # Return POP, total profit, and total trade count (for filtering)
    return (pop, total_profit, total_trades)
...
def genetic_optimize(df: pd.DataFrame, population_size: int, generations: int, mutation_rate: float) -> Tuple[Dict[str, Union[int, float]], float, float]:
    """Run the genetic algorithm and return the best individual and its metrics.

    This version optimises for total profit rather than probability of profit. It returns
    a tuple of (best_individual, best_total_profit, best_pop).
    """
    # Initialize population
    population: List[Dict[str, Union[int, float]]] = [random_individual() for _ in range(population_size)]

    # Evaluate initial fitnesses
    fitness_cache: Dict[int, Tuple[float, float, int]] = {}
    ...
    for gen in range(generations):
        ...
        if (gen + 1) % max(1, generations // 10) == 0:
            ...
            print(f"Generation {gen+1}/{generations}: best profit = {best_score:.4f}")
    ...
    final_metrics = [compute_fitness(df, ind) for ind in population]
    # index 1 is total profit
    total_profits = [m[1] for m in final_metrics]
    best_idx = int(np.argmax(total_profits))
    best_individual = population[best_idx]
    best_total_profit = total_profits[best_idx]
    # Recompute POP for the best individual
    best_pop = final_metrics[best_idx][0]
    return best_individual, best_total_profit, best_pop


def genetic_optimize_multi(dfs: List[pd.DataFrame], population_size: int, generations: int, mutation_rate: float) -> Tuple[Dict[str, Union[int, float]], float, float]:
    """Genetic optimisation across multiple datasets.

    The fitness of a candidate is the arithmetic mean of the total profits across all
    datasets. Probability of profit is still computed but not optimised on.

    Returns a tuple (best_individual, best_average_profit, best_average_pop).
    """
    # Initialise population
    population: List[Dict[str, Union[int, float]]] = [random_individual() for _ in range(population_size)]

    def evaluate(indiv: Dict[str, Union[int, float]]) -> float:
        metrics = [compute_fitness(df, indiv) for df in dfs]
        # metrics is list of (pop, total_profit, trades)
        metrics_used = [m for m in metrics if m[2] >= 12]
        profits = [m[1] for m in metrics_used]
    ...
    for gen in range(generations):
        ...
        parents = [ranked_pairs[0][1], ranked_pairs[1][1]]
        if (gen + 1) % max(1, generations // 10) == 0:
            print(f"Generation {gen+1}/{generations}: best average profit = {ranked_pairs[0][0]:.4f}")
        new_population = []
        for _ in range(population_size):
            child = crossover(parents[0], parents[1])
            mutate(child, mutation_rate)
    ...
    # Final evaluation using the same average-profit metric
    final_avg_profits = [evaluate(ind) for ind in population]
    best_idx = int(np.argmax(final_avg_profits))
    best_individual = population[best_idx]
    best_average_profit = final_avg_profits[best_idx]
    # Compute average POP for the best individual across datasets with sufficient trades
    best_metrics = [compute_fitness(df, best_individual) for df in dfs]
    best_metrics_used = [m for m in best_metrics if m[2] >= 12]
    best_pops = [m[0] for m in best_metrics_used]
    best_average_pop = (sum(best_pops) / len(best_pops)) if best_pops else 0.0
    return best_individual, best_average_profit, best_average_pop
...
def parse_args() -> argparse.Namespace:
    ...
    # IV and profit/loss options
    parser.add_argument('--iv_manual', type=float, default=None,
                        help=("Manual implied volatility to use if IB API is unavailable and the CSV lacks an IV column. "
                              "If provided, this value (e.g. 0.25 for 25%%) will be used for all rows."))
    parser.add_argument('--iv_default', type=float, default=0.25,
                        help=("Fallback implied volatility to use when no other IV data is available. "
                              "Default: 0.25 (25%%)."))
    parser.add_argument('--use_ib_iv', action='store_true',
                        help=("Attempt to fetch implied volatility from the Interactive Brokers API. "
                              "If this fails or is unavailable, the script falls back to manual or default IV."))
...
def main() -> None:
    ...
    if args.data:
        ...
        # Genetic optimisation returns best parameters, total profit and POP
        best_params, best_profit, best_pop = genetic_optimize(df, args.population, args.generations, args.mutation)
        ...
        print(f"\nTotal profit: {best_profit:.4f}")
        print(f"Probability of profit (POP): {best_pop:.2%}")
    elif args.folder:
        ...
        for sector in sectors:
            ...
            # Genetic optimisation returns best parameters, average total profit and average POP
            best_params, best_avg_profit, best_avg_pop = genetic_optimize_multi(dfs, args.population, args.generations, args.mutation)
            ...
            for key in output_order:
                if key in best_params:
                    print(f"  {key}: {best_params[key]}")
            print(f"\nAverage total profit across sector: {best_avg_profit:.4f}")
            # Report POP averaged across datasets that produced at least 12 trades
            # Also report how many datasets contributed to POP
            contrib_counts = sum(1 for df in dfs if compute_fitness(df, best_params)[2] >= 12)
            print(f"Average probability of profit (POP) across sector (on {contrib_counts}/{len(dfs)} datasets with >=12 trades): {best_avg_pop:.2%}")
