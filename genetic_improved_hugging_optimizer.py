...
# ---------------------------------------------------------------------------
def compute_fitness(df: pd.DataFrame, params: Dict[str, Union[int, float]]) -> Tuple[float, float]:
    ...
    # Return both POP and total profit so the GA can optimise on profit
    return (pop, total_profit)
...
def genetic_optimize(df: pd.DataFrame, population_size: int, generations: int, mutation_rate: float) -> Tuple[Dict[str, Union[int, float]], float, float]:
    ...
    fitness_cache: Dict[int, Tuple[float, float]] = {}
...
def genetic_optimize_multi(dfs: List[pd.DataFrame], population_size: int, generations: int, mutation_rate: float) -> Tuple[Dict[str, Union[int, float]], float, float]:
    ...
    def evaluate(indiv: Dict[str, Union[int, float]]) -> float:
        ...
        # metrics is list of (pop, total_profit)
        profits = [m[1] for m in metrics]
    ...
    # Final evaluation
    final_metrics = [compute_fitness(df, ind) for ind in population]
    average_profits = [m[1] for m in final_metrics]
    best_idx = int(np.argmax(average_profits))
    best_individual = population[best_idx]
    best_average_profit = average_profits[best_idx]
    # Compute average POP for the best individual
    pops = [m[0] for m in final_metrics]
    # We should recompute POP across all dfs for best individual
    best_pops = [compute_fitness(df, best_individual)[0] for df in dfs]
...
def main() -> None:
    ...
    if args.data:
        ...
        for sector in sectors:
            ...
            print(f"Average probability of profit (POP) across sector: {best_avg_pop:.2%}")
