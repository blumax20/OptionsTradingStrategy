...
### Fix FD: Accurate Attempts Logging + Health Report — Regular-Close Labeling/Detail, Submitted/Closed/Rejected Sections, Correct P/L, Retry 201-Rejected Opens (Jul 31)
**Status:** IMPLEMENTED (DCM committed; Health.ps1 both copies deploy-only, per the DS2/DS3 "deployed only — not in git repo" convention — they carry the live-port `$IB_PORT=7496` override)

**Incident:** On 2026-07-31 the health report (`health_20260731_103927.txt`) looked broken to the user: "Last 20 Orders Submitted" was empty, "Last 20 Closed" stopped at 7/24 even though HSBC & LNC had closed that morning with real realized P/L, and the P/L "didn't make sense" (suspected paper trading). Investigation (all confirmed in logs/CSVs):
- **Not paper.** Only port 7496 (live) listens; `Health.ps1` connects to `$IB_PORT=7496` everywhere; account `U21901574` (live). The P/L numbers were the real live account.
- **HSBC & LNC were REGULAR closes** (genuine CLOSE webhooks at 16:03–16:05 on 7/30), filled during RTH 7/31. Day Realized −64.35 ≈ their close → the P/L Summary was essentially correct; the **order lists** failed to corroborate it.
- **Root cause — the 5 PM reconcile logs regular closes as bare rows.** `_reconcile_positions_with_signals_lookback()` runs before the from-signal close delegate; when a held symbol's latest signal is CLOSE it closes via `_try_close_from_positions()` and wrote `close,placed,reconcile_close_signal, source=dcm-reconcile` with **only `limit` populated** — no `right/longK/shortK/order_action/exp/qty` (`DailyCycleManagement.py:3373`). The label also implied a flip/mismatch when it was an ordinary close.
- **"Last 20 Closed" (Fix DE, attempts-CSV based) dropped them** on three filters (reason not `success|worthless`, `order_action != SELL`, empty `longK`).
- **"Last 20 Submitted" empty:** the probe read only ephemeral live IB state (fresh clientId=900 `reqAllOpenOrders`+`openTrades`, 2 s, **no `reqOpenOrders()` reclaim** — Fix CZ/DA gap) + `reqExecutions` 48 h fills (none in window). The persistent attempts CSVs held every placement.
- **H's Error 201 (7/30)** = **insufficient margin** ("Equity with Loan Value [-65.04 USD] must exceed new total Initial Margin"; sibling LTH shows full text) — not PDT/permissions. Logged `H,open_call,error,bs_rejected_bag_e201` and **never retried** (Fix CP only collected `skipped/no_viable_limit_or_conditions|wide_spread_after_hours`).

**DCM changes (committed):**
- **D1 — Regular closes logged with full detail + honest reason.** `_try_close_from_positions()`'s `_place_combo()` now captures the closed legs (extends the Fix BV `nonlocal` capture) into `self._last_close_detail = {right,longK,shortK,exp,order_action,qty}`. The reconcile close `_AttemptLogger.write` (~line 3395) passes those through, so the row is as rich as PlaceAnOrder's `force_close,placed,success`. The `latest_is_close` reason is relabeled `reconcile_close_signal` → **`close_signal`** (a normal signal-driven close); `reconcile_flip_*`/`reconcile_mismatch` remain for genuine orientation changes. (`_flatten_stock_if_present`'s STK-flatten default reason is a different context and unchanged.)
- **D2 — Retry 201-rejected opens.** `_retry_skipped_opens_from_prev_day()` (10:00 AM, Fix CP) now also collects `open_*,status=error,reason startswith "bs_rejected_bag_e201"`. Position-guarded by PlaceAnOrder (held → skip); live `mid` pricing. **Caveat:** while Equity-with-Loan-Value is negative a margin 201 re-rejects — one bounded attempt/symbol/morning, effective once funded or if the 201 was transient.

**Health.ps1 changes (both copies; deploy-only):**
- **H1 — "Last 20 Submitted" from the attempts CSV** (walk back ~4 day-folders; `open_*`/`close_*` with status `placed|submitted|success`, newest 20). Added `ib.reqOpenOrders()` after `reqAllOpenOrders()` (Fix CZ/DA reclaim) so currently-working orders annotate `[working]`. Never empty when placements exist. (The bin copy's older `ib.trades()`/`legs_for_combo` probe was replaced with this block.)
- **H2 — New "Rejected / Errored Orders (today)"** section reads today's attempts CSV for `status=error` (+ hard-failure `skipped` reasons: `bs_rejected|no_viable_limit|opposite_unwind_failed|place_failed|qualify_failed`). Surfaces H/LTH `bs_rejected_bag_e201`. Read-only.
- **H3 — "Last 20 Closed" matches reconcile/roll closes.** Reason whitelist broadened to `success|worthless|close_signal|reconcile_close_signal|roll_mkt_close`; empty `order_action` treated as SELL; when `right`/`longK` are missing, **symbol-only fallback** to the most recent prior `open_*,success`. Header relabeled "(est. P/L from limits)"; per-row P/L now shows **losses in parentheses** (`P/L=($106.00)`, gains `P/L=$14.00`). Retroactively displays today's minimal HSBC/LNC rows.
- **H4 — P/L Summary via `ib.reqPnL()`** (`realizedPnL/unrealizedPnL`, `cancelPnL`), falling back to the accountValues `RealizedPnL/UnrealizedPnL` scrape if the stream yields `nan`. **Day Total = Realized + Unrealized** (so it always adds up — `dailyPnL` was dropped because it does not equal that sum). All values rounded to 2dp and formatted via `_fmtpl` — **negatives shown in parentheses** e.g. `(645.88)`. Labels `Realized (day)/Unrealized (open)/Day Total/YTD change NetLiq` (the non-ASCII `Δ` removed). YTD baseline unchanged; end-of-run `$lastPL` echo pattern updated.
- **H5 — "Last 10 submitted orders" (under Recent PlaceAnOrder activity) sourced from the attempts CSV.** The ib_cycle.log tail window (~2500 lines of a 535 MB verbose log) often doesn't reach back to the 5 PM batch, so the log-parse list was empty. Now reads today+2 prior day-folders' `attempts_*.csv` (open/close, status `placed|submitted|success`), newest 10. (Repo copy only — the bin copy lacks this section.)
- **H6 — "Recent Logs" always includes `ib_cycle.log` and excludes the Health self-log.** The section took the 3 most-recently-modified `*.log`, but the Health run's own `Health_*.log` (position dumps) is always newest and crowded out the real activity. Now `ib_cycle.log` is always first, plus the 2 next-most-recent excluding `Health_*`, and `ib_insync.wrapper:(position|updatePortfolio|openOrder)` / `Synchronization` noise is filtered from every shown log.
...
### Fix FG: Snap Invalid OTM Strike to the Chosen Expiry (WBD 25.5-that-doesn't-exist) (Aug 5)
**Status:** IMPLEMENTED

**Incident:** WBD PUT_OPEN (Aug 3, exp `20260918`) was written by the listener as `atm_strike=26.0, otm_strike_put=25.5`. WBD's Sep monthly is $1-wide (24/25/26/27) — **25.5 exists only on the weekly expiries**. IB returns `Error 200: No security definition ... strike=25.5 P` for the monthly (also 28.5 C, 23.5 P). The OTM leg couldn't be quoted → its ba% came back blank → the open was skipped `oi_below_threshold:ba=139.5/blank`, and the 10:00 AM retry re-used the same 25.5 and failed identically. A valid WBD trade was silently dropped every cycle.

**Root cause — two gaps:**

1. **Listener (source of the bad strike).** `_collect_secdef()` pools strikes across *all* expiries into one `strikes_all` list (IB's `reqSecDefOptParams` returns a flat union — no per-expiry grouping). For a PUT_OPEN the OTM leg is `put_otm_strike = _next_lower_existing(strikes_all, atm_strike)` (`listener.py` ~line 1143) → picks `25.5` (a weekly strike). The put-leg qualify loop (~line 1147-1152) only set `option=None` on failure — it **never snapped `put_otm_strike`** to a strike valid for the chosen expiry, so the invalid 25.5 was written to the CSV. (The call OTM had a single-shot snap fallback; Fix EB validated the **ATM** only, not the OTM legs.)

2. **Morning retry (why it didn't repair it).** `populate_missing_strikes` only flagged a row when OTM strikes were **empty** — `otm_missing = otm_call_missing AND otm_put_missing` (`LiquidityFilter.py` ~line 451, Fix CP-A). WBD's `otm_put=25.5` is present-but-invalid → the trigger never fired → 25.5 survived the retry.

**Fix — two parts:**

**FG-1 (`listener.py`): validate/snap OTM legs against the chosen expiry.** New helper `_snap_otm_to_expiry(ib, symbol, expiry, start, right, direction, strikes_all, tc, mult)` (after `_qualify_with_fallback`): if `start` qualifies for `expiry`, return it unchanged (common case, no behavior change); else walk the pooled strikes in `direction` (`'below'` for puts, `'above'` for calls), qualifying each candidate against the expiry, and return the first that qualifies (else `start`). Applied right after each OTM selection:
- Put OTM (~line 1146): `put_otm_strike = _snap_otm_to_expiry(..., 'P', 'below', ...)` → WBD 25.5 → 25.0.
- Call OTM (~line 1078): `otm_strike = _snap_otm_to_expiry(..., 'C', 'above', ...)` (belt-and-suspenders; the single-shot call fallback could also land on another weekly-only strike).
Logs `[WBD] Fix FG: OTM P 25.5 not valid for 20260918 — snapped to 25.0`.

**FG-2 (`LiquidityFilter.py`): repair present-but-invalid OTM in the morning retry.** In `populate_missing_strikes`, the first-pass scan now also collects open rows (CALL_OPEN/PUT_OPEN) with a present signal-relevant OTM strike into `otm_check_candidates` (early-return only when BOTH `rows_needing_strikes` and `otm_check_candidates` are empty). After connecting, each candidate's OTM strike is qualified against its expiry (one `qualifyContracts` per open row via the `Option(symbol, exp, k, right, "SMART", "100", "USD")` pattern reused from Fix FE-5); a strike that fails is added to `rows_needing_strikes` so `_get_atm_and_otm_strikes()` (which re-snaps via `_qualified_strikes_for_expiry`, Fix FE-5) rewrites both OTM legs to the valid grid. ~a dozen qualifies at 9:45/10:00/16:45; negligible.

**What this does NOT change:**
- Symbols whose pooled OTM already exists for the chosen expiry (the common case) — one qualify succeeds, no snap, no behavior change.
- ATM selection (Fix EB), expiry selection (Fix EA), CLOSE-signal strike handling (uses position strikes) — unchanged.
- Note: `populate_missing_strikes` now connects to IB whenever open rows exist (previously only when strikes were empty) — needed to validate present OTM strikes.

**Verification:** `python -m py_compile listener.py LiquidityFilter.py` OK. From `ib_cycle.log`, WBD `20260918` rejects 25.5 P / 28.5 C / 23.5 P (Error 200) while 25.0/26.0/27.0 qualify → walk-below from 25.5 lands on 25.0. Next WBD-type PUT_OPEN: listener writes `otm_strike_put=25.0` and the open places (or skips on a real OI/ba reason, not a phantom strike). Retry repair: `populate_missing_strikes` on a CSV with a present-but-invalid OTM flags and rewrites the row. Requires an OptionsListener restart for FG-1 to take effect.

---

### Fix FH: Prevent Duplicate Close Orders from Manual Starts/Restarts (Aug 6)
**Status:** IMPLEMENTED

**Incident:** On Aug 5, EIX (long 72.5P/short 70P) and NI each received **3** SELL close orders instead of 1. The 17:00 reconcile placed the correct close for each (EIX order 5578 @1.60). Then a manual menu run (PushButtonStart / `--after-hours`) fired at ~20:36 — right after an IB Gateway 2FA cold restart (login completed 20:36:26) — and placed 2 more `close_put` orders per symbol (@1.60 at 20:36:43, @0.61 at 20:38:11). Against a 1-spread position, 2–3 SELL closes filling would leave the account net short. (User cancelled the extras manually.)

**Root cause:** The menu run's *own* reconcile actually behaved correctly — it detected both working orders and skipped (`Reconcile ... skipping CLOSE for EIX/NI (already submitted/working)`, `submitted 0 CLOSE order(s)`). The duplicates came from **separate PlaceAnOrder invocations** whose close guard **missed the existing order**. Both guards — `has_working_auto_close()` ([ib_close_guard.py](InteractiveBrokersTrader/ib_close_guard.py)) and `_has_working_close_order()` (DCM) — did `reqAllOpenOrders()` then **slept 1.5s once** (Fix AP) before reading `openTrades()`. Right after an IB Gateway restart, 1.5s is too short for IB to sync existing open orders to the guard's *fresh* connection, so the scan found nothing → returned `False` → a duplicate close went out. This is the chokepoint every close passes through (menu 2, menu 4, scheduled).

**Fix — two parts:**

**Part A — poll-until-found in both close guards (the core fix).** Replaced the single 1.5s sleep with a poll loop that calls `reqAllOpenOrders()` + `sleep(dwell)` for `dwell in (1.5, 1.5, 1.0, 1.0)` (~5s total), **returning the instant a working close is found**. The per-trade scan logic was refactored into a local `_scan(trades)` (SELL BAG combo per Fix AB6, or individual OPT SELL per Fix AV1; working = PreSubmitted/Submitted/PendingSubmit/ApiPending, or Inactive+GTC/DAY). Timing: a symbol that already has a close returns on the first poll (~1.5s, unchanged); only a symbol with **no** existing close pays the full ~5s — which is exactly when we must be certain IB has finished syncing. Applies to `ib_close_guard.py` and `DailyCycleManagement.py:_has_working_close_order`. Because it queries IB (the source of truth), it protects **every** process — menu 2, menu 4, scheduled, concurrent — not just one cycle.

**Part B — in-cycle belt-and-suspenders.**
1. When the reconcile skips a symbol as already-working (`_reconcile_positions_with_signals_lookback`, ~line 3426), it now adds the symbol to `self._submitted_close_syms` so downstream in-cycle close paths (delegate stages per Fix AI2) skip it without re-hitting IB.
2. `_after_hours_batch_placement()` seeds `self._submitted_close_syms` from `_working_close_limit_symbols()` at the very start (~line 2933). This covers **menu 4** (`--after-hours`), which calls the batch directly and does **not** run the reconcile first, so it otherwise had nothing populating the set.

**What this does NOT change:**
- The common case (symbol already has a close) is unchanged in speed (~1.5s early-exit).
- Preclose cancel-replace (`_pre_close_market_conversion`) is separate and unaffected.
- The credit/inverted sweep still runs, but its PlaceAnOrder subprocess now hits the hardened Part A guard, so it can't place a duplicate either.
- No schema/task/service changes; both files are live-trading Python.

**Verification:** `python -m py_compile ib_close_guard.py DailyCycleManagement.py` OK. Next manual start/restart (menu 2 or 4) that runs while a working close exists: `ib_cycle.log` shows `Fix FH: seeded N symbol(s) with working closes into skip set: ...` and the close guard finds the existing order within the ~5s poll → `close,skipped,existing_working_close` (or `working_close_order`) instead of a duplicate `close_*,placed,success`. The EIX/NI scenario (manual run right after a gateway restart) no longer double-places.

---

### Fix FI: Consolidate Spread-CSV OI/IV Columns + Real Per-Leg IV in Theo Pricing (Aug 6)
**Status:** IMPLEMENTED

**Location:** `InteractiveBrokersTrader/theo_pricing.py` (NEW); `InteractiveBrokersTrader/LiquidityFilter.py` (`enrich_combined_csv`, `_ib_fetcher_factory._fetch`, new `_held_option_sides`, callers `enrich_if_rth`/`__main__`); `InteractiveBrokersTrader/DailyCycleManagement.py` (`_find_csv_oi` ~L4335, `_lookup_oi` ~L4722).

**Two problems in `combined_listener_spreads.csv`:**

1. **Dead parallel columns.** The listener header already carries the canonical set —
`iv_atm`, `iv_otm`, `open_interest_{atm,otm}_{call,put}` — but the enrichment appended a second,
inconsistent set (`oi_atm`, `oi_oth`, `iv_oth`, cols 36-38). They drifted out of sync (one CSV:
EIX had OI in both `open_interest_atm_call=88` and `oi_atm=88`, NI in neither, EHC only canonical;
`iv_otm` was **always empty**, IV split across `iv_atm`/`iv_oth`). Readers disagreed on which to
trust: the DCM low-OI cancel (`_find_csv_oi`/`_lookup_oi`) preferred the dead `oi_atm`/`oi_oth`,
while `_oi_ok()` read canonical.

2. **Theo ignored real IV.** `*_debit_theo_*` is computed once at signal time and after hours
mostly has no IV, so it priced with a flat **0.25** vol on both legs (6/9 rows one evening); the
enrichment captured per-leg IV at 16:45 (Fix FE frozen) but wrote it to the dead `iv_oth`, which
never fed the theo. `iv_otm` (Fix P skew) never engaged.

**Data-quality proof (justifies trusting captured IV):** across 49 rows / 8 days the signal-time
`iv_atm` (~16:01, ATM leg) and the independent enrichment IV (16:45, OTM leg) agree to a **median
0.0047** vol; flat 0.25 is off **~9.4 vol pts**. The only noisy outliers (SAN/PUK/VIV/STRC) are
illiquid ADRs with 166-200% bid-ask — already skipped by the Fix FC ba% gate before they trade.

**⚠️ Two different CSVs — not crossed:** the **attempts** CSV (`attempts_*.csv`) has its OWN
`oi_atm`/`oi_otm`/`threshold` columns (Fix EZ: PlaceAnOrder `ATTEMPT_FIELDS`; DCM `_AttemptLogger`
/ `_live_oi`). That is a logging schema for a different file — **left entirely untouched.** Only
the spread CSV's dead `oi_atm`/`oi_oth`/`iv_oth` are consolidated.

**Fix — five parts:**

**FI-1 (`theo_pricing.py`, NEW):** pure `_norm_cdf`/`_bs_price`/`_theo_spread_debits` (copied from
listener.py — skew-aware, Fix Y2 `>=0` + Fix CY `<=0.75*W` clamps), imported by LiquidityFilter
only. The listener can't be imported (`util.startLoop()`+`IB()` at module load) and is **left
untouched** → no listener change, no restart.

**FI-2 (LiquidityFilter `enrich_combined_csv` — writer to canonical):** `need_cols` drops
`oi_atm`/`oi_oth`/`iv_oth` (keeps `iv_atm`/`iv_otm`/`open_interest_*`/`ba_pct_*`); on read it
**strips** the dead cols from `cols` so any old CSV is cleaned on the next write. The fetch trigger
keys off canonical (`open_interest_atm_{call|put}` + `iv_atm`/`iv_otm`); ATM OI → the right-aware
`open_interest_atm_{call|put}`, short-leg IV → **`iv_otm`** (was `iv_oth`), OTM OI →
`open_interest_otm_{call|put}`. Fix AO Part-3 put-proxy (call OI → put cols) kept. The
"always-fresh" property of the old `oi_atm` column is preserved because after hours `iv_*` is
empty → the leg is fetched → canonical is overwritten with the fresh value.

**FI-3 (theo recompute):** after the leg fills, when `current_price`+`atm_strike`+`days_to_exp`
are present and **>=1 real IV** was captured, recompute the row's side theo:
`sigma_atm = iv_atm or iv_otm or 0.25`, `sigma_otm = iv_otm or iv_atm or 0.25`, `T = days_to_exp/365`,
`_theo_spread_debits(...)` → overwrite `{call|put}_debit_theo_{1,2_5,5}` (only the side matching
`right`). Uses the row's stored signal-time `current_price` — never refetches the stock. Fix CY/Y2
clamps still bound the output.

**FI-4 (frozen IV re-fetch, `_fetch`):** extended the Fix FE frozen block — when the type-4
`modelGreeks.impliedVol` is missing, `reqMarketDataType(2)` re-fetches greeks (last regular-session
= true pre-close IV) alongside the bid/ask. New `_clean_iv()` clamps to `0.03<=iv<=2.0`. No extra
calls when type-4 already returned IV.

**FI-5 (CLOSE rows — resolve the one held side):** `enrich_combined_csv` gained an optional `ib=`
param (passed by `enrich_if_rth` and the `__main__`/16:45 path; `None` for offline). Instead of
`elif right=="CLOSE": continue`, it resolves the held side via new `_held_option_sides(ib)` (one
`ib.positions()` call, cached; position is still open until the 17:00 close) and recomputes only
that side. Both sides held (rare roll) → both; no position / `ib is None` → keep the `continue`.
This also gives CLOSE rows the OI/ba enrichment they never got (harmless — FC gates are OPEN-only).

**FI-6 (DCM readers → canonical only):** `_find_csv_oi` and `_lookup_oi` alias lists drop
`oi_atm`/`oi_oth` (and stale `atm_oi`/`oi1`/`oi2`); canonical `open_interest_*` right-aware with
put-proxy-from-call fallback. Gate decisions unchanged — the values are the same daily OI, and the
one threshold-crossing mismatch found in 10 days of history (GILD OTM 103 vs 3) had empty `iv_otm`,
so the new code re-fetches and overwrites canonical with the fresh 103.

**What this does NOT change:** listener (no restart), PlaceAnOrder `_oi_ok` (already canonical),
the attempts-CSV schema, the FC ba%/OI OPEN-gates, preclose/reconcile/risk-exit/roll/STK-flatten.
At 9:45/10:30 RTH the `*_limit_*` columns are overwritten with live prices (Fix N) and PlaceAnOrder
prefers `limit` over `theo`, so the recompute is load-bearing mainly for the **5 PM after-hours**
batch.

**Verification:** `python -m py_compile theo_pricing.py LiquidityFilter.py DailyCycleManagement.py`
OK. Gate-equivalence over 10 days: 1 threshold-crossing OI mismatch (re-fetched). Dry-run against a
copy of `26_08_06` CSV: canonical OI/IV written, `iv_otm` populated, per-side theo recomputed,
CLOSE rows resolved to the correct held side (EHC→C, ITT→P, MET→C), dead cols stripped (39→36),
idempotent on re-run. Theo unit: skew `_theo_spread_debits(...,0.35,sigma_otm=0.33)` differs from
